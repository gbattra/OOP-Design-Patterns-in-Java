...
Each extends `IGear` with no additional methods. These serve keep the implementation
...
- `IPlayerBuilder addHandGear(GearType type, int attack, int defense, String adjective, String noun)` <br>
- `IPlayerBuilder addFootGear(GearType type, int attack, int defense, String adjective, String noun)` <br>
...
- `IBattle addHeadGear(GearType type, int defense, String adjective, String noun)` <br>
- `IBattle addHandGear(GearType type, int attack, int defense, String adjective, String noun)` <br>
- `IBattle addFootGear(GearType type, int attack, int defense, String adjective, String noun)` <br>
Methods to add `IGear` to the battlefield. Takes a `GearType`, the initial
`defense` and `attack` values for the gear, and the `adjective` and `noun` descriptors
of the gear.
...
- `List<IGear> getGears()` <br>
Returns the list of gear in the battle.

- `IBattle dressPlayers()` <br>
Dresses the players in the available gear. Algorithm attempts to give each
player the best possible gear from the available gear. Players go one at 
a time and pick the best gear available until there are no more items left.
Criteria is: if a gear can be combined, choose it; else, pick the gear with the
highest attack; else pick the gear with the highest defense.
